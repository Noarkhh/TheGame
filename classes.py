...
class HUD:
    ...
    def make_button(self, contents_surf, button_topleft, method, value, button_key, button_hover_key):
        button_surf = self.button_dict[button_key].copy()

        contents_rect = contents_surf.get_rect(centerx=button_surf.get_width() / 2, top=4)
    ...
    def load_menu(self, gw, button=None, value=None, press_hold=None):
        ...
        for h, (button_name, method) in enumerate(self.button_properties):
            ...
            self.make_button(text_surf, (40, 28 + (self.button_dict["button_"].get_height() + 4) * h),
                             method, None, "button_", "button_hover")
        gw.screen.blit(self.surf, self.rect)
    ...
    def load_savefile_menu(self, gw):
        ...
        for h in range(5):
            ...
            self.make_button(text_surf, (40, 28 + (self.button_dict["button_small"].get_height() + 4) * h),
                             savefile_choose, h, "button_small", "button_small_hover")
...
class BuildMenu(HUD):
    def __init__(self, gw):
        ...
        self.hover_surf.set_colorkey((255, 255, 255), RLEACCEL)
        self.press_surf = pg.transform.scale(pg.image.load("assets/hud/hud_tile_horiz_press.png").convert(), (108, 120))
        self.press_surf.set_colorkey((255, 255, 255), RLEACCEL)
        self.buttons = set()
        lowest = 0
        ...
        for i, building in enumerate(gw.key_structure_dict.values()):
            ...
            curr_hover_surf.blit(pg.transform.scale(new_build.surf, (60, 60 * new_build.surf_ratio[1])),
                                 (24, 8 + height))
            curr_press_surf = self.press_surf.copy()
            curr_press_surf.blit(pg.transform.scale(new_build.surf, (60, 60 * new_build.surf_ratio[1])),
                                 (24, 8 + height))

            new_button = Button(pg.Rect((self.rect.x + 36 + i * 104, 44), (108, 108)),
                                self.assign, type(new_build), curr_hover_surf, curr_press_surf)
...
class Snapper(Structure):
    ...
    def to_json(self):
        ...
        # "inside": self.inside,
        #
        # "snapper_dict_key": self.snapper_dict_key,
