...
def area_ghost_factory(struct_manager: StructManager, cursor: Cursor,
                       tile_entity: Type[T]) -> Optional[AreaGhost[T]]:
    if not tile_entity.is_draggable:
        return None
    if issubclass(tile_entity, Demolisher):
        return RectangleGhost(cursor, cursor.pos, tile_entity, AreaDemolisher(struct_manager))
    if issubclass(tile_entity, Structure):
        if tile_entity.drag_shape == DragShape.LINE:
            return LineGhost(cursor, cursor.pos, tile_entity, AreaBuilder(struct_manager, tile_entity))
        if tile_entity.drag_shape == DragShape.RECTANGLE:
            return RectangleGhost(cursor, cursor.pos, tile_entity, AreaBuilder(struct_manager, tile_entity))
    return None
